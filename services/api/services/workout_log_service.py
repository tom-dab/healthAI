...
class WorkoutLogService(BaseService):
    ...
    def get_log(self, db: Session, log_id: UUID):
        return self.get_or_404(db, log_id, 'Workout log not found')

    def update_log(self, db: Session, log_id: UUID, data: dict):
        return self.update(db, log_id, data, 'Workout log not found')

    def delete_log(self, db: Session, log_id: UUID) -> None:
        self.delete(db, log_id, 'Workout log not found')
